...
//小红点直径
#define PointMark_W_H 6


//TabBarButton中 图片与文字上下所占比
static const float scale=0.55;

#pragma mark-@interface SLTabBarButton
...
#pragma mark-@interface SLTabBar
@interface SLTabBar ()
...
/**
 *  若想外部代码更简洁,可调alloc init 初始化tabbar : SLTabBar *tabbar = [[SLTabBar alloc] init];但必须在这里初始化相关数据,如下:
 */
-(void)initData
{
    /*
...
-(void)createTabBarView
{
    ...
    for(int i=0;i<num;i++)
    {
        SLTabBarButton *button = [[SLTabBarButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/num*i, 0, [UIScreen mainScreen].bounds.size.width/num,self.tabBarHeight)];
        button.tag = 1000+i;
...
-(void)showControllerIndex:(NSInteger)index
{
    ...
    if([self.SLTabBarDelegate respondsToSelector:@selector(SLTabBar:didSelectViewController:)])
    {
        [self.SLTabBarDelegate SLTabBar:self didSelectViewController:self.viewControllers[index]];
    }
}
/**
 *  数字角标
